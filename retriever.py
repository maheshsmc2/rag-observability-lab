...
# Day 46: corpus hash to detect any change
...
def _load_artifacts():
    ...
    Day 46: add corruption safety, model/corpus/version checks,
    and auto-rebuild on mismatch.
    """
    # If any artifact missing → build fresh
    ...
    # Try to load artifacts; rebuild on corruption
    try:
    ...
    # --- Day 46: Self-healing consistency checks ---

    # 1) Index version
    if meta.get("index_version") != INDEX_VERSION:
        print(
            f"[WARN][Day 46] Index version mismatch: "
            f"meta={meta.get('index_version')} expected={INDEX_VERSION}. Rebuilding..."
        )
        return _build_and_save_index()
    ...
    if meta.get("model_name") != MODEL_NAME:
        print(
            f"[WARN][Day 46] Model changed "
            f"(meta={meta.get('model_name')}, current={MODEL_NAME}). Rebuilding..."
        )
        return _build_and_save_index()
    ...
    if meta.get("corpus_hash") != CORPUS_HASH or meta.get("n_docs") != len(DOCUMENTS):
        print(
            "[WARN][Day 46] Corpus changed (hash or doc count mismatch). "
            "Rebuilding index..."
        )
        return _build_and_save_index()

    # 4) Dimension from meta vs embeddings
    if meta.get("dim") != emb.shape[1]:
        print(
            f"[WARN][Day 46] Dimension mismatch in metadata: "
            f"meta_dim={meta.get('dim')} emb_dim={emb.shape[1]}. Rebuilding..."
        )
        return _build_and_save_index()
...
# 1. Dimension checks
assert (
    MODEL_DIM == DOC_EMBEDDINGS.shape[1] == faiss_index.d == META["dim"]
), (
...
assert META["n_docs"] == len(DOCUMENTS), (
    f"[ERROR][Day 45] Document count mismatch: "
    f"meta={META['n_docs']}, corpus={len(DOCUMENTS)}"
)

# 3. Norm check
_doc_norms = np.linalg.norm(DOC_EMBEDDINGS, axis=1)
assert np.allclose(
    _doc_norms.mean(), 1.0, atol=1e-2
), "[ERROR][Day 44] Corpus embeddings are not approximately L2-normalized."


# ---------------------------------------------------------
# 1.2 Lexical index: single BM25-style vectorizer
# ---------------------------------------------------------
...
def _encode_query_dense(query: str) -> np.ndarray:
    ...
    # Day 44: query norm check
    q_norms = np.linalg.norm(q_emb, axis=1)
    assert np.allclose(
        q_norms, 1.0, atol=1e-3
    ), "[ERROR][Day 44] Query embedding not L2-normalized."
...
def dense_search(query: str, top_k: int = 5) -> List[Dict]:
    ...
    Output format: list of {"id", "text", "score_dense"} dicts.
    """
...
# ---------------------------------------------------------
# 3.1 Lexical retrieval (BM25-like)
# ---------------------------------------------------------

def bm25_search(query: str, top_k: int = 5) -> List[Dict]:
    """
    Return top_k documents by BM25-like lexical similarity.
    Output format: list of {"id", "text", "score_bm25"} dicts.
    """
...
def compute_hybrid_vector(
    ...
    """
    Day 54: Centralized hybrid score engine.

    1) Min–max normalize both score arrays
    2) Blend using: alpha * lexical_norm + (1 - alpha) * dense_norm

    Returns:
        1D numpy array of hybrid scores.
    """
    dense_norm = min_max_norm(dense_scores)
    lex_norm = min_max_norm(lexical_scores)

    # alpha = lexical weight, (1 - alpha) = dense weight
    hybrid = alpha * lex_norm + (1.0 - alpha) * dense_norm
    return hybrid
...
def hybrid_search(
    ...
    """
    Combine dense + BM25 scores using weighted sum:
    ...
    hybrid_scores = compute_hybrid_vector(
        ...
        results.append(
            {
                "id": id_,
                "text": entry["text"],
                "score_hybrid": float(hybrid_scores[i]),
...
def rerank_with_cross_encoder(
    ...
    """
    Day 56: Rerank candidate docs using a CrossEncoder.

    Each candidate is a dict with at least {"id", "text", ...}.
    We attach "score_rerank" and sort by it.
    """
    if not candidates:
        return []
    ...
    # 2) Predict scores (higher = more relevant)
    scores = reranker.predict(pairs)

    # 3) Attach scores
    for cand, s in zip(candidates, scores):
        cand["score_rerank"] = float(s)
...
def hybrid_then_rerank(
    ...
    1) Use hybrid_search to get `retrieve_k` candidates (fast).
    2) Rerank those candidates with CrossEncoder (slow but accurate).
    3) Return top `final_k` docs.

    This is what your RAG app should normally call.
    """
    # Step 1: fast hybrid retrieval
    candidates = hybrid_search(query, top_k=retrieve_k, alpha=alpha)

    # Step 2: slow but smart reranking
...
def dense_scores_all(query: str) -> np.ndarray:
    """
    Compute dense similarity score for *all* documents using FAISS.
    Returns a vector of shape (N_docs,) aligned with DOCUMENTS.
    """
    q_emb = _encode_query_dense(query)
...
def compute_scores(
    ...
    """
    Compute all score vectors for a query:
    - dense_raw, dense_norm
    - bm25_raw, bm25_norm
    - hybrid_scores (with dynamic alpha or override)
    Also returns alpha used for hybrid.
    """
    query = query.strip()
    if not query:
    ...
    dense_raw = dense_scores_all(query)  # (N_docs,)
    dense_norm = min_max_norm(dense_raw)

    # BM25 / TF-IDF scores for all docs
    q_vec = BM25_VECTORIZER.transform([query])  # (1, V)
    bm25_raw = (BM25_MATRIX @ q_vec.T).toarray().ravel()  # (N_docs,)
    bm25_norm = min_max_norm(bm25_raw)

    # Dynamic alpha (short vs long query), unless overridden
    if alpha_override is not None:
        alpha = float(alpha_override)
    else:
        num_words = len(query.split())
        if num_words <= 3:
            alpha = 0.8  # short / keyword-y → lexical heavier
        else:
            alpha = 0.2  # longer / semantic → dense heavier

    # Hybrid scores (lexical vs dense) – Day 54 engine
    hybrid_scores = compute_hybrid_vector(
        dense_scores=dense_raw,
        lexical_scores=bm25_raw,
        alpha=alpha,
    )

    # Optional: debug for inspection
    print("\n[HYBRID DEBUG][Day 54]")
    print("alpha:", alpha)
...
def _select_best_doc(hybrid_scores: np.ndarray) -> int:
    """
    Threshold-based selection: keep docs >= (mean - std),
    then pick the best among them. If none, take global max.
    """
...
def answer_query(query: str) -> str:
    """
    Hybrid retriever:
    - Dense similarity with MiniLM + FAISS
    - Lexical similarity with TF-IDF (BM25-style)
    - Dynamic alpha (short vs long query)
    - Dynamic threshold on hybrid scores
    """
    txt = query.strip()
    if not txt:
    ...
    # Dynamic threshold (for debug printing)
    mean = float(hybrid_scores.mean())
    std = float(hybrid_scores.std())
    thr = max(0.0, min(mean - std, 1.0))
    ...
    return (
        f"Answer: {best_doc}\n\n"
        f"[debug] alpha={alpha:.2f}, "
        f"dense={best_dense:.3f}, "
        f"bm25={best_bm25:.3f}, "
        f"hybrid={best_hybrid:.3f}, "
        f"threshold={thr:.3f}"
    )
...
def retrieve_top_k(query: str, k: int = 3) -> Dict[str, object]:
    """
    Return top-k documents ranked by hybrid score, for analysis.
    """
...
def retrieve_with_trace(
    ...
    *,
    ...
    Logs:
      - dense_search top_k=retrieve_k
      - bm25_search top_k=retrieve_k
      - hybrid_search top_k=retrieve_k (includes score_hybrid, score_dense, score_bm25)
      - optional rerank_with_cross_encoder to final_k
    Saves trace JSON to trace/trace_<timestamp>.json
    """
